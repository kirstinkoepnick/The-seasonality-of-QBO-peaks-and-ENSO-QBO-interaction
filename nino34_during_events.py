...
def run_one_model(model: str, nboot: int, ci: float, qbo_threshold: float, seed: int | None):
    ...
    if (n_qbow + n_qboe) < min_events_total:
        raise ValueError(
            f"{model}: Too few QBO events ({n_qbow+n_qboe}) "
            f"using 1σ threshold ({qbo_thr:.2f} m/s)."
        )

    # Final masks
    qboe_mask = qbo_3m < -qbo_thr #easterly phase
    qbow_mask = qbo_3m > qbo_thr  #westerly phase
